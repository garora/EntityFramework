...
namespace Microsoft.Data.Entity.Migrations.Infrastructure
{
    public class Migrator
    {
            ...
            bool removeHistoryRepository)
        {
            ...
            {
                var sourceDatabase = new DatabaseModel();
                var targetDatabase = ModelDiffer.DatabaseBuilder.GetDatabase(HistoryRepository.HistoryModel);
                var ddlSqlGenerator = DdlSqlGeneratorFactory.Create(sourceDatabase, targetDatabase);
            ...
            foreach (var index in downgradeIndexes)
            {
                ...
                var targetDatabase = ModelDiffer.DatabaseBuilder.GetDatabase(GetSourceModel(migrations, index));
                var ddlSqlGenerator = DdlSqlGeneratorFactory.Create(sourceDatabase, targetDatabase);
            ...
            foreach (var index in upgradeIndexes)
            {
                ...
                var targetDatabase = ModelDiffer.DatabaseBuilder.GetDatabase(migration.TargetModel);
                var ddlSqlGenerator = DdlSqlGeneratorFactory.Create(sourceDatabase, targetDatabase);
            ...
            if (historyRepositoryExists && removeHistoryRepository)
            {
                ...
                var targetDatabase = new DatabaseModel();
                var ddlSqlGenerator = DdlSqlGeneratorFactory.Create(sourceDatabase, targetDatabase);
